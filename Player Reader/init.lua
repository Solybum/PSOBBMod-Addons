...
local function SaveOptions(options)
    ...
    if file ~= nil then
        ...
        for i=1,4,1 do
            ...
            io.write(string.format("            HideWhenMenuUnavailable = %s,\n", tostring(options.players[i].HideWhenMenuUnavailable)))
            io.write(string.format("            Changed = %s,\n", tostring(options.players[i].Changed)))
            io.write(string.format("            Anchor = %i,\n", options.players[i].Anchor))
            io.write(string.format("            X = %i,\n", options.players[i].X))
            io.write(string.format("            Y = %i,\n", options.players[i].Y))
...
local function present()
    ...
    if playerIsInDedicatedLobby then
        if (options.singlePlayersEnableWindow == true)
            and (options.singleHideWhenMenu == false or lib_menu.IsMenuOpen() == false)
            and (options.singleHideWhenSymbolChat == false or lib_menu.IsSymbolChatOpen() == false)
            and (options.singleHideWhenMenuUnavailable == false or lib_menu.IsMenuUnavailable() == false)
        then
            for i=1,4,1 do
                if options.players[i].EnableWindow then
                    local address = lib_characters.GetPlayer(i - 1)
